...
> Контекст:
> - `_tasks_/phase_04/Phase_4_vision_final.md` — зафиксированные решения
> - `_tasks_/phase_03/001_plan_phase_03.md` — образец формата плана фазы
> - `_checkpoints_/20260719_004_checkpoint.md` — текущее состояние проекта
>
> Дата: 2026-07-20
...
> **Уточнение из реализации:** ключ — чистая функция от сигнатуры (не от соседей).
> Поэтому синглтон-функция с аргументами `sp_y(uuid)` получает `/signature/<hash>` в ключе,
> хотя её **файл** остаётся коротким (`function sp_y.sql`). Различие между «когда суффикс
> в ключе» (всегда при непустой сигнатуре) и «когда суффикс в имени файла» (только при
> коллизии с перегрузкой) — это намеренная дизайн-инварианта, зафиксированная в
> `Phase_4_vision_final.md` §4.2.
...
**Файлы:** `_tasks_/phase_04/Phase_4_vision_final.md` (уже зафиксировано в §6).
...
**Файл:** `_checkpoints_/20260720_001_checkpoint.md` + коммит
...
Документы (1–3) и код (4–7) не смешивать в одном коммите — по конвенции `_tasks_`.
