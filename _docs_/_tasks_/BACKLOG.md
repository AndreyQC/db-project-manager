...
## P2. `codebase doctor` — прогон по autodoc-заголовкам: проверка + дозаполнение сигнатур/хешей

**Контекст (пользователь, 2026-09-14, находка шага 16.9-prep):** кодовые базы,
сгенерированные старыми версиями инструмента (или через `yaml apply`), несут
неполные autodoc-заголовки — в частности, у функций нет `object_signature` /
`argument_types`, а `object_key` без `/signature/<hash>`. Свежий RE их пишет —
в результате identity двух сторон не совпадает и компаратор «расщепляет» объекты
на added+removed пары (67 функций на cis_zup, 2026-09-14). Аналогично могут
отставать и другие поля autodoc.

**Действие:** команда вида `db-pm codebase doctor --dir <codebase>`:
1. обходит все `*.sql` с autodoc, проверяет целостность заголовка:
   `object_key` соответствует фактическим schema/type/name файла, сигнатура
   функции вычислима из тела, `object_signature`/`argument_types` согласованы;
2. пересчитывает сигнатуры из SQL-тела (`signature_hash(argument_types)`) и
   дозаполняет недостающие поля autodoc (`object_signature`, `/signature/<hash>`
   в `object_key`, переименование файла при коллизиях перегрузок);
3. dry-run по умолчанию + `--fix` для записи; отчёт что изменено.

**Предварительное условие:** сначала закрыть 16.9 (вычисление сигнатуры из тела
при отсутствии в autodoc на стороне снапшота) — doctor переиспользует тот же
экстрактор аргументов.

**Альтернатива без инструмента:** перегенерация кодовой базы свежим RE — но это
массовая перезапись пользовательских файлов (потеря ручных правок formatting/
комментариев вне autodoc) и требует живой БД.

**Связано:** `infrastructure/sql/autodoc.py` (`update_header`),
`infrastructure/parsing/pg_sql_parser.py` (vertex key из autodoc),
`infrastructure/diff/comparator.py::identity_key`, Phase 16 журнал §4 (16.9).

**Не блокирует деплой, но снижает шум diff'а для старых кодовых баз.**

---

## P3. Канонизация рендера выражений каталогом GP6 (9 view cis_zup)

**Контекст (Phase 16, 2026-09-13):** остаточные CHANGED по view —
`pg_get_viewdef` GP 6 (ядро PG 9.4) рендерит арифметику дат развёрнутыми
CAST-формами: `CAST(CAST('3000-01-01' AS DATE) AS TIMESTAMP) - CAST('1 day' AS
INTERVAL)` vs рукописное `CAST('3000-01-01' AS DATE) - INTERVAL '1 day'`.
Пары конечны и перечислимы — канонизировать по образцу правил Phase 16.7
(normalize_sql, построчная верификация живых пар).

**Триггер:** замечание из прогона, когда шум view начнёт мешать.

**Связано:** `infrastructure/diff/normalize_sql.py`, Phase 16 журнал §4 (16.6–16.7).

---

## P3. qualify-refs не должен трогать комментарии/строки в телах функций

**Контекст (Phase 16, 2026-09-13):** 4 функции cis_zup реально «CHANGED» из-за
того, что qualify-refs при деплое квалифицировал имена таблиц ВНУТРИ
комментариев plpgsql-тел (`-- Очистка таблицы stg_x` → `... cis_stg_zup.stg_x`).
RE-снапшот законно отражает БД, кодовая база — исходный текст: вечный дрейф.

**Действие:** в qualify-refs (regex-проход) исключить `--`-комментарии и
строковые литералы из переписывания.

**Связано:** `application/qualify_refs_service.py`.

---

## P3. Safety-отчёт: подсказка «presence: unknown → выполните ANALYZE»

**Контекст (Phase 16.11, 2026-09-15):** свежесозданные таблицы на GP без
статистики (до ANALYZE) fail-safe-блокируют свои будущие дропы — «таблицы с
данными» при count(*)=0. Пользователь разбирается вручную.

**Действие:** в safety-gate отчёт/лог добавлять рекомендацию ANALYZE для
таблиц с `presence: unknown`/`stale`; кандидат на включение в Phase 17
(post-deploy шаг с ANALYZE).
