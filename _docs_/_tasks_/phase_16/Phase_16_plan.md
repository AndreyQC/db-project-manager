...
> **Статус:** ЗАВЕРШЕНА (шаги 16.1–16.12; журнал §4 — нормативная хронология)
> **Результат:** `Phase_16_result.md`
> **Фаза в `_phases_`:** `Phase_16.md`
...
| 16.11 | 2026-09-15 | `deploy apply` (прогон пользователя, отклонён CD-10) | 32 таблицы «непокрытые с данными» при попытке дропа (`--include-drops`); параллельно — просьба: отклонённый apply не сообщает путь к артефактам | Диагноз: все 32 пусты (count(*)=0), но GP не ANALYZE-ил свежесозданные таблицы → reltuples-статистика stale/unknown → fail-safe SG-4 «unknown = есть данные». Поведение `--include-drops` корректно: флаг согласие на включение дропов в план, НЕ обход data-gate (CD-9/CD-10). Фикс UX: CLI `deploy plan`/`apply` печатают путь к артефактам при rejected/error. Разблокировка: ANALYZE 32 таблиц выполнен → план 412 safe/0/0, gate CLEAN | Unit: 2 теста расширены (артефакт-строка при отклонении plan/apply); 1033 passed; ruff clean; живой план после ANALYZE: gate CLEAN, touched 0 | **готово** |
| 16.12 | 2026-09-15 | логи прогонов (замечание пользователя) | Шум: `'CREATE TABLE … (period_calc DATE NULL, …' contains unsupported syntax. Falling back to parsing as a 'Command'` — 1176 раз за `deploy plan` | Диагноз по стеку/форме строки: `snapshot.py:87` звал `sql_hash(normalized)` — двойная нормализация; канонический GP-хвост (`… distributed randomly`, без `;`) при повторном парсе деградировал до Command (§72-1). Фикс: `hash_normalized()` — чистый SHA-256 по уже-нормализованной строке (без парса); канонический хвост получает `;` → `normalize_sql` идемпотентна; Command-узел уходит на детерминированный regex-путь вместо сырого текста | Unit: 3 теста (идемпотентность, консистентность hash, отсутствие повторного парса); 1036 passed; ruff clean. Живой `deploy plan`: **1176 → 0 предупреждений**, сравнения стабильны (unchanged/changed без изменений) | **готово** |
