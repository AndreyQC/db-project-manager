...
- **Integration (testcontainers):** (1) таблица с данными + add nullable column → plan: safe,
  ...
  дельте → rollback, повторный apply идемпотентен (pre скипается); (5) DDL, из которого
  колонки не извлекаются (напр. `PARTITION OF`-наследник) → `columns=None` → fail-safe
  needs-pre при данных; (6) тип-синонимы: `integer` в кодовой базе против `int4` в БД →
  TYPE_CHANGED **не** возникает.
