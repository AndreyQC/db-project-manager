...
class PgSqlParser(ObjectGraphParser):
    ...
    def _parse_file(self, path: Path, root: Path) -> tuple[Vertex | None, list[str], str]:
        ...
        if autodoc and autodoc.get("object"):
            ...
            # Phase 16.9: codebases generated by older tool versions carry no
            # signature in autodoc — the key has no /signature/ suffix, while a
            # fresh RE always writes one, so the comparator split the same
            # routine into added+removed pairs (67 functions on cis_zup,
            # 2026-09-14). Reconstruct from the SQL body: autodoc argument_types
            # (catalog form) wins, otherwise extract from the DDL header.
            if object_type in {"function", "procedure"} and not signature and "/signature/" not in object_key:
                if not argument_types:
                    argument_types = extract_argument_types(raw)
                if argument_types:
                    signature = signature_hash(argument_types)
                    object_key = f"{object_key}/signature/{signature}"
