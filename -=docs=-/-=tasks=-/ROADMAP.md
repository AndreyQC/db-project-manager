...
> сессиями. Дополняет `-=tasks=-/BACKLOG.md`:
>
...
> **Источник:** исходный набросок был в `-=PHASES=-/Future_phase.md` (удалён — нарушал
...
- **Q1** — Нужен ли отдельный edge diff (BACKLOG P2), если будет Delta Viewer?
  *Рекомендация:* включить в DV как вкладку «Рёбра», а не отдельной фазой.
- **Q2** — Delta Viewer: начинать с CLI или сразу GUI?
  *Рекомендация:* сначала CLI/markdown, GUI после.
