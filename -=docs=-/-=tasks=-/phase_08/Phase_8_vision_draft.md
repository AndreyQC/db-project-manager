# Phase 8: Overload resolution в edge detection — драфт (vision draft)
...
> - `-=tasks=-/ROADMAP.md` §2, шаг 1 — Phase 8 как следующая фаза (зависит от —)
> - `-=tasks=-/BACKLOG.md` P1 «Разрешение перегруженных вызовов в edge detection»
...
`/signature/<hash>`), так что перегрузки больше не затирают друг друга в графе (LESSONS §26).
Но edge-detection эту идентификацию **не использует**: `_build_names_index`
...
Это именно та «первая попавшаяся вершина», что зафиксирована в BACKLOG P1. Топосорт
деплоя для перегрузок формально строится, но рёбра недостоверны — а в ROADMAP §2 Phase 10
(CD Foundation) опирается на граф, и недостоверные рёбра −2 — это некорректный порядок
при развязке перегрузок через общие зависимости.

**Доп. препятствие (подтверждено исследованием кода):** чтобы выводить тип по аргументам
вызова, нужны **raw типы аргументов** перегрузки (`"int4"`, `"text,varchar"`). Сейчас:
adapter (`postgres/queries.py` GET_FUNCTIONS/PROCEDURES → `argument_types`) и generator
(`_function_ctx`/`_procedure_ctx` имеет типы в scope и хеширует их) — типы есть upstream.
Но в **autodoc** пишется только 8-hex `object_signature` (`autodoc.py:80-81`), а
`_render_kind` (`sql_generator.py:283-293`) не пробрасывает `autodoc_extra` для
function/procedure. Парсер (`pg_sql_parser.py:169`) читает только хеш; на `Vertex`
(`graph.py:38-59`) raw типов нет. Вывод по аргументам невозможен, пока типы не проброшены.

Скобки вызова `()` сносятся нормализатором до пробелов (`normalize.py:29-30`), поэтому
inference нельзя делать по нормализованному word-потоку — нужен regex-проход по **raw
SQL** файла (по образцу `QualifyRefsService` из §36).
...
2. Неразрешимые вызовы — детерминированный fallback (как сейчас, к одной перегрузке)
   + **transparent-протокол** (лог + файл), чтобы пользователь знал, чему доверять.
3. Raw типы аргументов доступны на графе (`Vertex.argument_types`) — без этого inference
   невозможен; поле нужно и для будущих фаз (edge diff P2, structural column-diff CD-ALT-1).
4. Регрессия: число рёбер на реальной кодовой базе **не уменьшается** (чеклист Phase 8).

## 3. Принятые решения (закрыты через Q&A с пользователем)

| Развилка | Решение | Обоснование |
|---|---|---|
| Гранулярность inference (MVP) | **Только литералы** | Колонки требуют индекса колонок (парсинг CREATE TABLE или доп. поля в autodoc таблиц) — заметно больший объём. Литералы закрывают ядро BACKLOG P1 минимально. Колонки/рекурсия вызовов — follow-up (см. §6). |
| Хранение raw типов | **Новое поле `Vertex.argument_types: str = ""`** + запись `object.argument_types` в autodoc | Typed-доступ; сериализуется в `.dbm_graph/` автоматически (`model_dump`/`model_validate`, без явного field-list). `extra="ignore"` на Vertex сейчас **дропает** unknown-ключи → поле обязательно добавить явно. |
| Протокол неразрешённых | **Лог (warning) + файл `_overload_resolution_report.md`** в корне codebase | По образцу `QualifyRefsService._write_report` (§36): аудит доверия к графу, persistent. Файл `.md` не попадает в SQL-скан. |
| Regex по raw SQL (не AST) | **Regex-MVP** | Согласуется с выбранным подходом qualify-refs (§36 tradeoff): покрывает function-call pattern `name(...)`, без новой зависимости. AST (sqlglot, уже в deps с Phase 9) — follow-up, если появятся false positives/negatives. |
...
4. **`!` `_render_kind` (283-293)** → пробросить `autodoc_extra={"argument_types": raw}` для
   function/procedure (механизм `extra` и коллизия-гард в `build_metadata` уже готовы)
...
> (поле аддитивное; graph_store версионирования проверяет только `"1"`).
...
- **Resolver** (новый модуль, напр. `infrastructure/parsing/overload_resolution.py`):
  - Принимает raw SQL текущего файла, имя функции, схему, список перегрузок.
  - Regex-поиск вызовов `(\bschema\.)?\bname\s*\((...)\)` в raw SQL (вне строковых
    литералов по возможности; CTE/dynamic-SQL — известные ограничения, как §36).
  ...
  - Сравнить `canonical_signature` выводимых типов с `canonical_types_tuple` каждой
    перегрузки. **Ровно 1 совпадение** → та перегрузка. 0 или >1 → unresolved.

### 4.3. Type inference MVP — правила (черновик для `_plan`)
...
> Правила уточняются в `_plan` на основе реальных перегрузок. Ключевое свойство: inference
> консервативен — при любой неоднозначности возвращает unknown (никогда не угадывает).
...
- **Не разрешено** (unresolved): файл, вызов, причина (ambiguous / 0 совпадений / нет
  аргументов / не-литерал), к какой перегрузке отнесено (fallback-first).
...
- **Новая фикстура** `tests/fixtures/codebase_sample/app/functions/function sp_x_caller.sql`
  (или отдельный каталог): тело вызывает `app.sp_x(123)` (→ int4) и `app.sp_x('literal')`
  (→ text). Assert: DEPENDS_ON-рёбра идут к `sp_x__19f12f3f` (int4) и `sp_x__982d9e3e`
  (text) соответственно — это разрыв, который сейчас не покрыт (исследование: ни одна
  фикстура не вызывает `sp_x`).
...
- **Регрессия «рёбра не убавились»**: на существующей фикстуре `codebase_sample` число
  рёбер до/после — не меньше.
- **(опц.) Integration**: testcontainers с перегрузками + вызовами — edge destination
  равен нужному overload-key. (Текущий `test_phase5_extensions_e2e.py` проверяет только
  наличие вершин, не рёбра — §BACKLOG P2 «интеграционный тест» закрыт, но edge-routing
  не покрыт.)
...
- **Колонки как аргументы** вызова (прямые ссылки на колонки таблиц с известным типом) —
  требует индекса колонок (CREATE TABLE regex или catalog). New BACKLOG-запись P2.
...
- **Edge diff** (сравнение рёбер графа между состояниями) — отдельный BACKLOG P2 (Phase 9
  follow-up), не часть Phase 8.
...
- [ ] §12: `git add -- "-=docs=-/-=tasks=-/phase_08/..."` (дефис в имени каталога).
...
## 8. USER_INPUT (рекомендации; большинство закрыто Q&A)

> Стиль ROADMAP §9 — рекомендации; если пользователь согласен, переношу в `_final`.

- **UI-1** Нужен ли CLI-флаг отключения resolution (`--no-overload-resolution`,
  fallback-first для всех)? *Рекомендация:* нет — поведение и так детерминированное
  (resolved → точное ребро, unresolved → fallback); флаг добавляет поверхность без выгоды.
- **UI-2** Формат отчёта — markdown-таблицы (как qualify-refs)? *Рекомендация:* да,
  единый стиль отчётов проекта.
- **UI-3** Размещение resolver-модуля — `infrastructure/parsing/overload_resolution.py`
  vs метод в `pg_sql_parser.py`? *Рекомендация:* отдельный модуль (тестируется изолированно,
  как `domain/signature.py`).
- **UI-4** Если для некоторой перегрузки `argument_types` пуст/отсутствует (нет в
  autodoc — старый файл или non-PG) — всегда unresolved? *Рекомендация:* да, fallback-first
  + запись в протокол (source = «no signature data»).

## 9. Где читать дальше

- `-=CHECKPOINTS=-/20260730_001_checkpoint.md` — текущее состояние проекта
- `-=tasks=-/phase_04/Phase_4_vision_final.md` — Phase 4 (идентификация перегрузок)
- `-=PHASES=-/Phase_09.md` — compare (SQL-нормализация через sqlglot, переиспользуется)
