...
>
> Предыдущий артефакт: `-=tasks=-/phase_08/Phase_8_vision_draft.md` (не удаляется —
...
> - `-=tasks=-/ROADMAP.md` §2, шаг 1 — Phase 8 как следующая фаза
...
- `-=tasks=-/phase_08/Phase_8_vision_draft.md` — предшествующий драфт (история обсуждения)
...
- `-=PHASES=-/Phase_09.md` — compare (SQL-нормализация через sqlglot)
