...
- **Панель «дерево проекта БД + просмотр SQL» (read-only preview).** Не замена SQL-IDE (DBeaver), а быстрая инспекция уже сгенерированных файлов, как панель проводника в редакторе кода:
  - слева — `QTreeView` с `QFileSystemModel` по корню выходной директории (или собственная `QAbstractItemModel` с иконками по типу объекта: table/view/function/...). Дерево ленивое — потянет тысячи файлов;
  - разделитель — `QSplitter` (перетаскиваемая граница, состояние сохраняется в конфиге);
  - справа — просмотрщик содержимого выбранного файла, read-only. Реализация — **`QScintilla`** (`PySide6` обёртка `QsciScintilla`): полноценная подсветка синтаксиса SQL, номера строк, сворачивание блоков, моноширинный шрифт;
  - по клику на узле дерева — асинхронное чтение одного файла (не блокирует UI);
  - это естественно дополняет reverse-engineer: сгенерировал файлы → тут же видишь их в дереве и можешь быстро глянуть содержимое.
- Зависимость: добавляем `PySide6-QScintilla` (или эквивалент `QScintilla` для текущей версии PySide6) в `pyproject.toml`.
