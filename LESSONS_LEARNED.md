...
## Phase 16 — Greenplum tuning (деплой на реальный GP-кластер)

### 70. Каталог-SQL обязан жить в пределах ядра старейшего таргета: GP 6 = PG 9.4
- **Симптом (cis_zup_gp_dev, 2026-09-09):** `db-pm reverse-engineer` падал на
  самой первой таблице (`__deploy.schema_version`):
  `psycopg2.errors.UndefinedFunction: function array_position(int2vector, smallint)
  does not exist` в `GET_INDEXES::ORDER BY`.
- **Диагноз (живой пробник через адаптер проекта):** `SELECT version()` →
  Greenplum 6.19.4, ядро **PostgreSQL 9.4.26**; `array_position` появилась в
  PG 9.5 → в `pg_proc` её нет вовсе. Единственное место в `queries.py` с
  конструкцией новее 9.4 (аудит grep'ом по всем функциям запросов).
- **Фикс (Phase 16.1, коммит `79bcf11`):** универсальный ORDER BY через
  скалярный подзапрос `(SELECT u.ord FROM unnest(idx.indkey::smallint[])
  WITH ORDINALITY AS u(k, ord) WHERE u.k = a.attnum)` — доступен с PG 9.4,
  работает и на PG, и на GP. Контракт-тест
  `test_indexes_query_avoids_pg95_array_position`.
- **Верификация на обеих СУБД:** (1) GP-кластер — замена выполняется;
  (2) PostgreSQL 18.6 (`cis_zup_dev`) — построчное сравнение старого и нового
  ORDER BY на всех таблицах с индексами: результаты идентичны.
- **Урок #1:** Greenplum — это НЕ «современный Postgres»: ядро GP 6 — PG 9.4,
  GP 7 — PG 12. Любой SQL к системному каталогу обязан обходить конструкции
  новее ядра старейшего поддерживаемого GP. Триггеры для аудита: `array_*`
  (9.5+), jsonb-функции (9.5+), `pg_sequence`/`pg_sequence_*` (10+),
  `MERGE` (15+).
- **Урок #2:** перед выбором замены — живой пробник (`SELECT version()`,
  наличие функции в `pg_proc`, кандидат-конструкция на реальном кластере).
  Текст ошибки пользователя — гипотеза (продолжение §69-3), подтверждай
  каталогом.
- **Урок #3:** эквивалентность замены доказывай построчным сравнением старой
  и новой формы на живом PG-корпусе, а не «запрос выполнился»: замена
  ORDER BY-выражения меняет порядок строк — это видно только диффом результатов.
- **Урок #4:** prefer универсальный SQL (одна форма для PG+GP); ветвление по
  `_is_greenplum` — только когда эквивалента нет (образец: `pg_sequence` →
  `GET_SEQUENCES_GREENPLUM`). Два запроса = две ветки тестов и два места
  рассинхронизации.

### 71. GP-админ-схемы не отделить ни `pg_%`-фильтром, ни extension-ownership — только списком схем
- **Симптом (cis_zup_gp_dev, 2026-09-13):** `deploy analyze` остановлен
  вердиктом VIOLATIONS: три web-таблицы `gp_toolkit`
  (`__gp_log_master_ext`, `__gp_log_segment_ext`, `gp_disk_free`) ушли в
  REMOVED с оценкой ~1e6 строк и «нет покрывающего pre-скрипта»; в RE-снапшот
  попала вся админ-схема (52 объекта, включая 27 функций и 45 view).
- **Диагноз (живой пробник):** фильтр «NOT LIKE `pg_%`» не отделяет
  `gp_toolkit` — префикс другой; объекты НЕ extension-owned (в `pg_extension`
  только `plpgsql`, 0/52 с `deptype='e'`), поэтому pg_depend-фильтр (как у
  функций) не срабатывает. Единственная точка отсечения — список схем
  (`_get_schemas` → единственный потребитель `get_database_structure`).
- **Фикс (Phase 16.5, коммит `e8234a9`):** `GP_ADMIN_SCHEMAS`
  (`gp_toolkit`, `gp_statistics`, `gp_statistics_history` — последние для
  GP 7, no-op на GP 6) исключаются в `_get_schemas()` **только при
  `_is_greenplum`** — на PostgreSQL одноимённая схема была бы пользовательской.
- **Урок #1:** у Greenplum есть админ-схемы вне `pg_%`/`information_schema` —
  при добавлении новой СУБД составляй список системных схем живым пробником
  (`pg_namespace` минус пользовательские), а не предполагай префикс.
- **Урок #2:** «системность» объекта определяется двумя независимыми
  механизмами — extension-ownership (`pg_depend.deptype='e'`) и принадлежность
  к админ-схеме; проверяй оба, прежде чем выбирать точку фильтра.
- **Урок #3:** GP-only знание (имена админ-схем) не должно протекать в
  универсальный SQL/семантику PG-пути — держи его в GP-ветке
  (`_is_greenplum`), даже когда универсальная форма формально no-op.

### 72. sqlglot молча деградирует незнакомое DDL до Command-узла; GP-каталоги живут в pg_catalog
- **Симптом (cis_zup_gp_dev, 2026-09-13):** 211 таблиц CHANGED при совпадающих
  версиях source/target — hash-сравнение шло по сырому тексту: sqlglot,
  встретив `WITH (...)`/`DISTRIBUTED ...` в CREATE TABLE, НЕ бросает
  исключение, а возвращает Command-узел с исходным текстом — обещанный
  regex-fallback (collapse+lower) не выполнялся вовсе, сравнение стало
  чувствительно к пробелам/кавычкам/регистру.
- **Фикс (Phase 16.6–16.7):** GP-хвост вырезается до парса и канонизируется
  отдельно; «голова» парсится полноценным AST. Верифицировано живым
  сравнением RE-снапшота с кодовой базой: таблицы 193/196 equal.
- **Урок #1:** «parse_one не упал» ≠ «AST построен» — проверяй тип корневого
  узла (Command = деградация), если нормализация обязана быть детерминированной.
- **Урок #2 (живой пробник):** у `gp_distribution_policy` в GP 6.19.4 колонок
  `attrnums` НЕТ — фактические: `localoid/policytype/numsegments/distkey/
  distclass`; `distkey` — int2vector (пустой = RANDOMLY). Документация
  новее кластера — всегда открывай фактические колонки через pg_attribute.
- **Урок #3:** GP-системные каталоги лежат в `pg_catalog`
  (`pg_catalog.gp_distribution_policy`); квалификация `gp_catalog.*` даёт
  UndefinedTable. «gp_»-префикс имени ≠ имя схемы.
- **Урок #4:** канонизации семейства «каталог ↔ рукописный DDL» конечны и
  перечислимы (bool/boolean, timezone(...)/AT TIME ZONE, дефолтный VOLATILE,
...
### 74. DROP FUNCTION на ядрах < PG 10 требует список аргументов даже пустым: «syntax error at or near X» указывает позицию парсера, а не причину
- **Симптом (cis_zup_gp_dev, 2026-09-24):** `deploy reset` (content-drop)
  падал на объекте №139; повторный прогон — №1 (счётчик сместился: первые
  138 объектов уже удалены, это был один и тот же объект — alphabetically
  первый routine). Первая гипотеза — «CASCADE в DROP FUNCTION нелегален на
  ядрах < PG 13» — опровергнута живым прогоном без CASCADE: ошибка
  сместилась на следующий токен — `syntax error at or near ";"` после
  `DROP FUNCTION IF EXISTS "s"."fun_lu_orbit_constructor_zon_del"`.
- **Корневая причина:** `pg_get_function_identity_arguments` для zero-arg
  функции возвращает `''`; код рендерил ident без скобок (`if args:`).
  Ядра < PG 10 (GP 6 = 9.4) в грамматике DROP FUNCTION требуют
  `name(arg_list)` — даже пустого; PG 10+ скобки опускает молча, маскируя
  баг на новых таргетах. Со скобками `name() CASCADE` легален на любом ядре.
- **Фикс:** routines в `drop_schema_contents` всегда рендерят аргументы
  (`()` для zero-arg); CASCADE для routines сохранён. Временный
  версионный гейт по server_version_num (введён по первой гипотезе)
  удалён; бонус возврата CASCADE — остаточный риск aggregate↔support-функции
  снимается сам (CASCADE уводит agg, IF EXISTS гасит miss).
- **Урок #1:** «syntax error at or near X» называет токен, НА котором
  парсер споткнулся, а не виновника: X сразу после имени функции = парсер
  ждал `(`. Смещение X после правки (CASCADE → `;`) — маркер, что причина
  не тронута, двигался только симптом.
- **Урок #2:** пустой рендер identity-списка (`''`) ≠ «клаузу опустить»:
  в DDL с позиционным списком аргументов пустой список всё равно
  рендерится — `name()`.
- **Урок #3:** гипотеза подтверждена только когда ошибка ушла ЦЕЛИКОМ, а
  не сместилась на соседний токен; «убрали X — ошибка ушла с X» — типичная
  ложноположительная верификация.
